...
function displayList(items, wrapper, rowsPerPage, page) {
  ...
  for (i = 0; i < paginatedItems.length; i++) {
    ...
    newDiv.innerText = paginatedItems[i].name;
    newDiv.classList.add("list-items");
    wrapper.appendChild(newDiv);
  }
}

function displayPages(items, wrapper, rowsPerPage) {
  wrapper.innerHTML = "";
  let pageCount = Math.ceil(items.length / rowsPerPage);
  for (i = 0; i <= pageCount + 1; i++) {
    if (i == 0) {
      let prev = "prev";
      let btn = createButton(prev, items);
      wrapper.appendChild(btn);
    } else if (i == pageCount + 1) {
      let next = "next";
      let btn = createButton(next, items);
      wrapper.appendChild(btn);
    } else {
...
function createButton(page, items) {
  ...
  button.addEventListener("click", function (e) {
    if (e.target.innerText == "prev") {
      ...
      displayList(items, lists, rows, currentPage);
      let currentBtn = document.querySelector(".btn.active");
      currentBtn.classList.remove("active");
      let newButton = currentPage;
...
displayList(pagination, lists, rows, currentPage);
displayPages(pagination, pages, rows);
